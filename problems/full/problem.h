#pragma once

// Full polynomial multiplication in 𝔽_q[x] with q = P^M.
//
//   T_full(N) : 𝔽_q[x]_<N × 𝔽_q[x]_<N → 𝔽_q[x]_<2N−1
//   (f, g) ↦ f · g
//
// As a tensor:
//   T_full = Σ_{i, j < N} a_i ⊗ b_j ⊗ c_{i+j}.

#include <format>
#include <string>

#include "core/constraints.h"
#include "core/gf_vec.h" // for IntPow
#include "core/tensor.h"
#include "problems/full/symmetry.h"
#include "problems/full/tensor.h"

namespace full {

template <int P, int M, std::size_t N> struct Problem {
  static_assert(N >= 1);
  static_assert(N <= 32); // backtracking ceiling for the M = 1 path
  static_assert(M >= 1);

  static constexpr int kNA = N;
  static constexpr int kNB = N;
  static constexpr int kNC = 2 * N - 1;
  static constexpr int kP = P;
  static constexpr int kM = M;
  static constexpr int kQ = static_cast<int>(IntPow(P, M));

  using Vec = GFVec<P, M, N>;
  using SymmetryGroup = full::SymmetryGroup<P, M, N>;

  static Tensor<P, M, kNA, kNB, kNC> MakeTensor() {
    return BuildMulTensor<P, M, N>();
  }

  static std::string Name() { return std::format("full_q{:02}_n{:02}", kQ, N); }
};

} // namespace full
